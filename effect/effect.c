...
static void sample_move_d16_sS (char *dst, float *src, unsigned long nsamples, unsigned long dst_skip)
{
	while (nsamples--) {
...
static void sample_move_dS_s16 (float *dst, char *src, unsigned long nsamples, unsigned long src_skip)
{
...
void effect_src(const float* const input0, const float* const input1, unsigned long nframes) {
	if (st_src) {
		sample_move_d16_sS((char*)st_src->sampv, (float*)input0, nframes, 4);
                      ...
                      auplay_write_h *wh, void *arg)
{
	...
	if (!stp || !ap || !prm)
        ...
        if ((st_play = mem_zalloc(sizeof(*st_play), auplay_destructor)) == NULL)
		                return ENOMEM;
	st_play->run = false;
	st_play->ap  = ap;
	st_play->wh  = wh;
